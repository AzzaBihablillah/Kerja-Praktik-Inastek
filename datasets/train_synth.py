"""Train YOLOv8n cap/no_cap on synth_cap_v3 (synthetic + colour-balanced conveyor).
...
def main():
    ...
    m.train(
        data=f"{DATASETS}/synth_cap_v3/data.yaml",
        imgsz=640, epochs=70, batch=16, patience=20,
        seed=0, device=0, workers=2, cache=False, close_mosaic=10,
        project=f"{DATASETS}/cap_runs", name="synth_cap_v3_640",
        plots=True, val=True, exist_ok=True,
    )
    best = f"{DATASETS}/cap_runs/synth_cap_v3_640/weights/best.pt"
    mm = YOLO(best)
    met = mm.val(data=f"{DATASETS}/synth_cap_v3/data.yaml", split="test",
                 project=f"{DATASETS}/cap_runs", name="synth_cap_v3_640_test",

...
def non_max_suppression(
    ...
    # second dimension is [each classes' probability],[xyxy]
    ...
    # for each images in the batch
    for xi, x in enumerate(prediction):

        # cat apriori
        # autolabel function will be implemented here
        #
        #
        #
        # endfunction
        ...
        # no need to do additional calculation in yolov8
        # print(x[:4, :].T)
        # print(x)
        # convert from xywh to xyxy
        box = xywh2xyxy(x[:4, :].T)

        # # this is for multilabel mode, but idc rn, only focusing on bess class rn
        # if multi_label:
        #     pass
        # else:

        # this one is for best class mode
        conf, j = x[4:, :].T.max(1, keepdim=True)
    ...
    # xc = prediction[..., 4:].amax(dim=-1) > conf_thres  # candidates
    # print('checkpoint')
    # print(prediction[..., 4:].amax(dim=-1))
...
def clip_coords(
    boxes: Union[torch.Tensor, np.ndarray], img_shape: Tuple[int, int]
) -> None:
